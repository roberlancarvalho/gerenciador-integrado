...
class MemberItem extends Component {
  ...
  render() {
    const {member, isOnline, position, onPressThreeDots} = this.props;
      ...
      <View style={[fontFamilyRegular, baseStyles.row, styles.container]}>
          ...
          <View style={[baseStyles.media, baseStyles.itemHorizontalCenter, {marginBottom: 0}]}>
            ...
            <View style={baseStyles.mediaBody}>
              <Text
                ...
                {getTitle(member.name_title) + member.name}
              </Text>
              {position ?
                <View>
        ...
        <View style={baseStyles.colOne}>
          <View style={{flexDirection: 'row', alignItems: 'center'}}>
            <TouchableOpacity style={{padding: padding.md}} onPress={() => this.onPressEmail(member.email)}>
              <Image
