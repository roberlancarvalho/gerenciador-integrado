...
class ViewGroupMembershipNotification extends Component {
  ...
  getUsers = () => {
    ...
    if (users) {
      ...
      const {user} = this.props.notification;
      return <Text key={user.id + "-user"} onPress={() => Actions.userProfile({user_id: user.id})}
                   style={[baseStyles.fontsLg, fontFamilyMedium]}> {user.name}</Text>
    }
    return "";
  };
  ...
  render() {
    ...
    return (
      <TouchableOpacity style={[styles.postContainer, baseStyles.media, baseStyles.alignItemsCenter]}
                        onPress={() => Actions.groupMemberships({group: this.props.notification.group})}>
        <View style={[{marginRight: margin.md}, baseStyles.mediaLeft]}>
          <AppAvatar
            size={50}
            rounded
            source={{uri: MEDIA_BASE_PATH + notification.user.profile_pic}}
            activeOpacity={0.1}
          />
        </View>
...
export default ViewGroupMembershipNotification;
